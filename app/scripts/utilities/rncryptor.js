...
RNCryptor.KeyForPassword = function(password, salt) {
  ...
  return sjcl.misc.pbkdf2(password, salt, 100000, 32 * 8, hmacSHA256);
}

RNCryptor.InternalKeyForPassword = function(password, salt) {
...
RNCryptor.Encrypt = function(password, plaintext, options) {
  ...
  var encryption_key = RNCryptor.InternalKeyForPassword(password, encryption_salt);

  var hmac_salt = options["hmac_salt"] || sjcl.random.randomWords(8 / 4);
  var hmac_key = RNCryptor.InternalKeyForPassword(password, hmac_salt);
